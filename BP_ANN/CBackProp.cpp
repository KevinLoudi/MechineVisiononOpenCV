...
CBackProp::CBackProp(int nl,int *sz,double b,double a):beta(b),alpha(a)
{
	...
	//	allocate memory for delta error value for each neuron
...
//This function updates the output value for each neuron.
//starting with the first hidden layer, it takes the input to each neuron and 
//finds the output by first calculating the weighted sum of inputs and 
//then applying the Sigmoid function to it.
